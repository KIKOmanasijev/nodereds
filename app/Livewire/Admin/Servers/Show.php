...
class Show extends Component
{
    ...
    public function deleteServer(): void
    {
        ...
        try {
            // Dispatch delete job
            DeleteServerJob::dispatch($this->server->id, Auth::id());

            $this->dispatch('notify', [
                'type' => 'success',
                'message' => 'Server deletion queued. The server will be deleted from Hetzner Cloud and database shortly.',
            ]);

            // Redirect to servers index
            $this->redirect(route('admin.servers.index'));
        } catch (\Exception $e) {
            Log::error('Failed to queue server deletion', [
                'server_id' => $this->server->id,
                'error' => $e->getMessage(),
            ]);
            ...
            $this->dispatch('notify', [
                'type' => 'error',
                'message' => 'Error queueing server deletion: ' . $e->getMessage(),
            ]);
        }
    }
